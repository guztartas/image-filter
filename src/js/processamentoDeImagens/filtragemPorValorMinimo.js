function filtragemPorValorMinimo(matrix, width, tamanhoDoFiltro) {
    ...
    for (let i = kernelSize; i < linhaEchunkSeparados.length - kernelSize; i++) {
        for (let j = kernelSize; j < linhaEchunkSeparados[i].length - kernelSize; j++) {
            ...
            imagem[i][j][0] = Math.min(...totalPixelSoma[0]);
            imagem[i][j][1] = Math.min(...totalPixelSoma[1]);
            imagem[i][j][2] = Math.min(...totalPixelSoma[2]);
            imagem[i][j][3] = 255;
        }
    }
